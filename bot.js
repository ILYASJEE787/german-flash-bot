...
app.post("/webhook", async (req, res) => {
  ...
  if (u.callback_query) {
    ...
    if (action === "approve") {
      ...
      await tgSend(chatId, `❌ <b>REJECTED!</b>\n\n👤 ${order.name}\n📦 ${order.plan}`);
...
app.post("/order", async (req, res) => {
  ...
  res.json({ ok: true, success: true }); // Added success: true for HTML compatibility
});

// ─── GET /status/:txid: Frontend calls this to check specific order ───
app.get("/status/:txid", (req, res) => {
  ...
  if (order) {
    ...
    res.json({ status: "pending", note: "Order not found in memory" });
  }
});
